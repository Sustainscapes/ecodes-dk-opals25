...
from opals import Cell
from opals import AddInfo
from opals import Types
from opals import pyDM
from opals import Algebra
...
def odm_import_single_tile(tile_id):
    ...
    try:
        # Import tile id
        import_tile = Import.Import()
        import_tile.inFile = laz_file
        import_tile.outFile = odm_file
        #import_tile.commons.screenLogLevel = Types.LogLevel.none
        import_tile.run()
...
def odm_generate_footprint(tile_id):
    ...
    try:
        ...
        #export_tif.commons.screenLogLevel = Types.LogLevel.none
        export_tif.run()
...
def odm_validate_crs(tile_id, mosaic = False):
    ...
    try:
        odm_dm = pyDM.Datamanager.load(odm_file)
        ...
        elif crs_str == 'COMPD_CS["ETRS89 / UTM zone 32N + DVR90 height",PROJCS["ETRS89 / UTM zone 32N",GEOGCS["ETRS89",DATUM["European_Terrestrial_Reference_System_1989",SPHEROID["GRS 1980",6378137,298.257222101,AUTHORITY["EPSG","7019"]],AUTHORITY["EPSG","6258"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4258"]],PROJECTION["Transverse_Mercator"],PARAMETER["latitude_of_origin",0],PARAMETER["central_meridian",9],PARAMETER["scale_factor",0.9996],PARAMETER["false_easting",500000],PARAMETER["false_northing",0],UNIT["metre",1,AUTHORITY["EPSG","9001"]],AXIS["Easting",EAST],AXIS["Northing",NORTH],AUTHORITY["EPSG","25832"]],VERT_CS["DVR90 height",VERT_DATUM["Dansk Vertikal Reference 1990",2005,AUTHORITY["EPSG","5206"]],UNIT["metre",1,AUTHORITY["EPSG","9001"]],AXIS["Gravity-related height",UP],AUTHORITY["EPSG","5799"]],AUTHORITY["EPSG","7416"]]':
            odm_dm.setCRS(settings.crs_wkt_opals)
            return_value = 'Tile: multiple - set'
        else:
    ...
    if mosaic == True:
        try:
            odm_dm = pyDM.Datamanager.load(odm_mosaic)
            crs_str = odm_dm.getCRS()
            # Check whether CRS exists, if not assign, if different throw error.
            if crs_str == settings.crs_wkt_opals:
...
def odm_export_normalized_z(tile_id):
    ...
    try:
        ...
        #export_normalized_z.commons.screenLogLevel = Types.LogLevel.none
...
def odm_export_canopy_height(tile_id):
    ...
    try:
        # Initialise exporter
        export_canopy_height = Cell.Cell()

        # Export mean
        export_canopy_height.inFile = odm_file
        export_canopy_height.outFile = out_file
        ...
        export_canopy_height.noData = -9999
        ...
        export_canopy_height.limit = 'corner' # This switch is really important when working with tiles!
                                    # It sets the ROI to the extent to the bounding box of points in the ODM
        #export_canopy_height.commons.screenLogLevel = Types.LogLevel.none
...
def odm_export_point_count(tile_id, name = 'vegetation_point_count',
    ...
    try:
        ...
        #export_point_count.commons.screenLogLevel = Types.LogLevel.none
...
def odm_calc_proportions(tile_id, prop_name, point_count_id1, point_count_id2):
    ...
    try:
        do_algebra = Algebra.Algebra()
        do_algebra.inFile = [num_file, den_file]
        do_algebra.outFile = out_file
        do_algebra.formula = "r[0]/r[1]"
        do_algebra.run()
...
def odm_export_amplitude(tile_id):
    ...
    try:
        ...
        # It sets the ROI to the extent to the bounding box of points in the ODM
        export_amplitude.commons.screenLogLevel = Types.LogLevel.none
...
def odm_export_point_source_info(tile_id):
    ...
    try:
        ...
        dm = pyDM.Datamanager.load(odm_file)
...
def odm_export_date_stamp(tile_id):
    ...
    try:
        ...
        add_GPSDate = AddInfo.AddInfo()
        add_GPSDate.inFile = odm_file
        add_GPSDate.attribute = '_GPSDay=floor(GPSTime/(60*60*24))'
        add_GPSDate.commons.screenLogLevel = Types.LogLevel.none
    ...
    try:
        ...
        export_time_stamp.commons.screenLogLevel = Types.LogLevel.none
